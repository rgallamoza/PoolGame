...
extern vector <Creature *> creature_array;
...
extern glm::vec3 cylinder_position;
...
void copy_flocker_states_to_graphics_objects()
{
  ...
  for (int i = 0; i < num_creatures; i++) {
    xyz_positions[i] = glm::vec3(creature_array[i]->position.x, creature_array[i]->position.y, creature_array[i]->position.z);
    //quat_orientations[i] = glm::normalize(glm::quat(glm::vec3(drand48() * 360.0, drand48() * 360.0, drand48() * 360.0)));
    glm::mat4 R = glm::mat4(glm::vec4(creature_array[i]->frame_x, 0),
                            glm::vec4(creature_array[i]->frame_y, 0),
                            glm::vec4(creature_array[i]->frame_z, 0),
                            glm::vec4(0, 0, 0, 1));
    quat_orientations[i] = glm::quat(R);

    // Set initial velocities
    xyz_velocities[i] = glm::vec3(creature_array[i]->velocity.x, creature_array[i]->velocity.y, creature_array[i]->velocity.z);

    // Convert velocity from per frame to per second
    xyz_velocities[i] *= target_FPS;
  }
}
...
{
  for (int i = 0; i < num_creatures; i++) {
    creature_array[i]->position.x = xyz_positions[i].x;
    creature_array[i]->position.y = xyz_positions[i].y;
    creature_array[i]->position.z = xyz_positions[i].z;

    // Copy orientation to flocker
    glm::mat4 R = glm::toMat4(quat_orientations[i]);
    creature_array[i]->frame_x = glm::vec3(R[0]);
    creature_array[i]->frame_y = glm::vec3(R[1]);
    creature_array[i]->frame_z = glm::vec3(R[2]);
  }
}
...
void bullet_add_obstacles()
{
  ...
  // add two walls on YZ plane
  btCollisionShape* wallYZ1Shape = new btStaticPlaneShape(btVector3(-1, 0, 0), -box_width);
  btCollisionShape* wallYZ2Shape = new btStaticPlaneShape(btVector3(1, 0, 0), 0);

  btDefaultMotionState* wallMotionState = new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(0, 0, 0)));

  btRigidBody::btRigidBodyConstructionInfo wallYZ1RigidBodyCI(0, wallMotionState, wallYZ1Shape, btVector3(0, 0, 0));
  btRigidBody* wallYZ1RigidBody = new btRigidBody(wallYZ1RigidBodyCI);
  bullet_dynamicsWorld->addRigidBody(wallYZ1RigidBody);

  btRigidBody::btRigidBodyConstructionInfo wallYZ2RigidBodyCI(0, wallMotionState, wallYZ2Shape, btVector3(0, 0, 0));
  btRigidBody* wallYZ2RigidBody = new btRigidBody(wallYZ2RigidBodyCI);
  bullet_dynamicsWorld->addRigidBody(wallYZ2RigidBody);

  // Add cylinder
  btCollisionShape* cylinderShape = new btCylinderShape(btVector3(cylinder_radius, box_height, cylinder_radius));

  btDefaultMotionState* cylinderMotionState = new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(cylinder_position.x, cylinder_position.y, cylinder_position.z)));
...
*/
  ...
  chShape->calculateLocalInertia(shapeMass, simplifiedShapeInertia);
  ...
  for (int i = 0; i < num_creatures; i++) {
    ...
    btRigidBody::btRigidBodyConstructionInfo rigidBodyCI(shapeMass,
							 motionstate,
							 chShape,//simplifiedShape,   // collision shape of body
							 simplifiedShapeInertia);

    btRigidBody *rigidBody = new btRigidBody(rigidBodyCI);
    btVector3 v = rigidBody->getLinearVelocity();
    v.setX(xyz_velocities[i].x);
    v.setY(xyz_velocities[i].y);
    v.setZ(xyz_velocities[i].z);
